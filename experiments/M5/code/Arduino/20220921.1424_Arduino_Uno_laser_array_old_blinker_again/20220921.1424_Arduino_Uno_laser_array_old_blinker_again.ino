...
void loop() {
  ...
  if (go) {
    ...
    wait_one_cycle();
    fire_lasers(1, 0, 0, 1); // write an LDI instruction here

    // 10: 1001 (immediate data)
    wait_for_rising_edge_of_bit(0); // accumulator contains 0001
    wait_out_SR_instruction();
    wait_into_NOP_instruction();
    ...
    wait_one_cycle();
    fire_lasers(1, 0, 0, 1); // set bits in ACC value to be 1001 for blinker
    
    // 11: another LDI instruction, using STA
    wait_for_rising_edge_of_bit(0); // accumulator contains 0001
    wait_out_SR_instruction();
    wait_into_NOP_instruction();
    fire_lasers(0, 1, 0, 0); // turn NOP into STA instruction
    ...
    wait_one_cycle();
    fire_lasers(0, 1, 1, 0); // set bits in ACC value to 0110 for blinker
