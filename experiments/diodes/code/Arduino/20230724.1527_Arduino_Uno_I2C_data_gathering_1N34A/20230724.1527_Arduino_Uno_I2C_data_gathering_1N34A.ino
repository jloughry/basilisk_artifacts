...
char wavelength_string[10] = "532nm";
...
const int relay_5V = 10; // relay control is active low
...
void setup() {
  ...
  motor_x->setSpeed(60); // RPM
  motor_y->setSpeed(60); // This is about as fast as it will go.
...
bool check_aim(void) {
  ...
  const double threshold = 2.8; // how far down the bus should be depressed, in volts
...
const double raster_width = 1.0; // mm
const double raster_height = 1.0; // mm
...
void legend(void) {
  ...
  Serial.print(",");
  Serial.println("measured_bus_voltage");
  Serial.println();
}
...
void line_scan(void) {
  ...
  for (int sample = 0; sample < samples_per_line; sample++) {
    ...
    Serial.print(",");
    Serial.println(measure_bus_voltage()); // actually make the measurement
    delay(100);
